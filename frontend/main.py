...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        if not got_artifact_update and last_task is not None:
            ...
            # If still no parts (e.g. error, or plain text), check the history
            if not parts and getattr(last_task, "history", None):
                for msg in reversed(last_task.history):
                    if getattr(msg, "role", None) == Role.agent and getattr(msg, "parts", None):
                        parts.extend(_extract_parts(msg.parts))
                        break
